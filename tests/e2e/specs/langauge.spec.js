...
describe("Manual Language Setting Test", () => {
  ...
  it("Visits the app root url changes the langauge reloads and checks that text changed", () => {
    ...
    cy.get("div.noDocs").contains(
      "You don't have any documents yet. Press new on the top right of your screen to create one.",
    );
    ...
    cy.get("div.noDocs").contains(
      "Você ainda não possui nenhum documento. Pressione novo no canto superior direito da tela para criar um.",
    );
    cy.reload();
    cy.get("footer.footer")
      .get(".local")
